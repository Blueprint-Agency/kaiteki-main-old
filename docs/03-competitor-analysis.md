...
**Net IA target:** `kaiteki.my/{treatments|concerns|branches|doctors|blog}/{slug}`, English-first under `/en/` with `/ms/` reserved, flat (≤3 clicks, all real `<a href>`), one intent per URL, concern→treatment→branch internal-linking clusters, and a medical-schema + named-reviewer layer on every page.
